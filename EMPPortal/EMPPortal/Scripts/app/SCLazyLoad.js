...
function getAllCustomerInformation(i) {
    ...
    var url = '/api/CustomerInformation/CustomerSearchInfoLazyLoad?UserName=' + $('#UserId').val() + '&Status=' + Status + '&SiteType=' + SiteType
    ...
    ajaxHelper(url, 'POST').done(function (data) {
        ...
        $("#tbCustomersBody").append(tableContent);
        dataCunt = dataCunt + 15;
    });
    $('#dvLoaderCust').hide();
    // $('table').footable();
}

$('#dvCustomers').on('scroll', function () {
    if ($(this).scrollTop() + $(this).innerHeight() >= this.scrollHeight) {
        $('#dvLoaderCust').show();
        getAllCustomerInformation(0);
        $('#dvLoaderCust').hide();
    }
});
